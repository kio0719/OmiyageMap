class HomeController < ApplicationController
  ...
  def index
    ...
    @results = @q.result(distinct: true).page(params[:page]).includes([images_attachments: :blob],
[user: [icon_attachment: :blob]], :likes).per(25)
    @count = @q.result.count
    @randoms = Post.includes([images_attachments: :blob],
:user).where.not(images_attachments: { id: nil }).order("RAND()").limit(5)
  ...
  def freeword_divise_search
    ...
    grouping_hash = key_words.reduce({}) do |hash, word|
      hash.merge(word => { name_or_address_or_caption_or_user_name_cont_any: word })
    end
